  ...
  <div class="evaluate-page">
    ...
    <el-card class="section-card">
      <template #header>
        <div class="card-header">
          <span
            ><el-icon><DataAnalysis /></el-icon> 在线检索评估</span
          >
        </div>
      </template>
      <p class="section-desc">
        输入查询，实时评估检索质量（无需 Ground Truth）
      </p>
      <div class="eval-form">
        <el-input
          v-model="onlineQuery"
      ...
      <div v-if="onlineResult" class="eval-result">
        <el-descriptions :column="3" border size="small">
          ...
          <el-descriptions-item label="最高分">{{
            onlineResult.max_rerank_score
          }}</el-descriptions-item>
          <el-descriptions-item label="最低分">{{
            onlineResult.min_rerank_score
          }}</el-descriptions-item>
          <el-descriptions-item label="置信度">{{
            onlineResult.avg_confidence
          }}</el-descriptions-item>
          <el-descriptions-item label="向量检索占比">
            <el-progress
              :percentage="Math.round(onlineResult.vector_ratio * 100)"
              :stroke-width="8"
            />
          </el-descriptions-item>
          <el-descriptions-item label="BM25检索占比">
            <el-progress
              :percentage="Math.round(onlineResult.bm25_ratio * 100)"
              :stroke-width="8"
              status="success"
            />
          </el-descriptions-item>
    ...
    <el-card class="section-card">
      <template #header>
        <div class="card-header">
          <span
            ><el-icon><Document /></el-icon> 离线评估（需 Ground Truth）</span
          >
        </div>
      </template>
      <p class="section-desc">
        提供检索结果和标准答案，计算 Recall/Precision/MRR/NDCG 指标
      </p>
      ...
      <div v-if="offlineResult" class="eval-result">
        <el-descriptions :column="4" border size="small">
          <el-descriptions-item label="Recall@1">{{
            offlineResult.recall_at_1
          }}</el-descriptions-item>
          <el-descriptions-item label="Recall@3">{{
            offlineResult.recall_at_3
          }}</el-descriptions-item>
          <el-descriptions-item label="Recall@5">{{
            offlineResult.recall_at_5
          }}</el-descriptions-item>
          <el-descriptions-item label="综合得分">
            <el-tag type="success">{{ offlineResult.retrieval_score }}</el-tag>
          </el-descriptions-item>
          <el-descriptions-item label="Precision@1">{{
            offlineResult.precision_at_1
          }}</el-descriptions-item>
          <el-descriptions-item label="Precision@3">{{
            offlineResult.precision_at_3
          }}</el-descriptions-item>
          <el-descriptions-item label="Precision@5">{{
            offlineResult.precision_at_5
          }}</el-descriptions-item>
          <el-descriptions-item label="MRR">{{
            offlineResult.mrr_at_k
          }}</el-descriptions-item>
          <el-descriptions-item label="NDCG@1">{{
            offlineResult.ndcg_at_1
          }}</el-descriptions-item>
          <el-descriptions-item label="NDCG@3">{{
            offlineResult.ndcg_at_3
          }}</el-descriptions-item>
          <el-descriptions-item label="NDCG@5">{{
            offlineResult.ndcg_at_5
          }}</el-descriptions-item>
          <el-descriptions-item label="MAP">{{
            offlineResult.map_at_k
          }}</el-descriptions-item>
        </el-descriptions>
      </div>
    </el-card>
    ...
    <el-card class="section-card">
      <template #header>
        <div class="card-header">
          <span
            ><el-icon><TrendCharts /></el-icon> 评估历史汇总</span
          >
      ...
      <div v-if="summary && summary.total_queries > 0">
        <el-descriptions :column="3" border>
          ...
          <el-descriptions-item label="平均Recall@5">{{
            summary.avg_recall_at_5
          }}</el-descriptions-item>
          <el-descriptions-item label="平均NDCG@5">{{
            summary.avg_ndcg_at_5
          }}</el-descriptions-item>
          <el-descriptions-item label="平均MRR">{{
            summary.avg_mrr
          }}</el-descriptions-item>
          <el-descriptions-item label="平均MAP">{{
            summary.avg_map
          }}</el-descriptions-item>
          ...
          <el-descriptions-item label="P50">{{
            summary.retrieval_score_p50
          }}</el-descriptions-item>
          <el-descriptions-item label="P90">{{
            summary.retrieval_score_p90
          }}</el-descriptions-item>
        </el-descriptions>
      </div>
    ...
    <el-card class="section-card">
      <template #header>
        <div class="card-header">
          <span
            ><el-icon><ChatDotRound /></el-icon> 用户反馈统计</span
          >
      ...
      <div v-if="feedbackStats && feedbackStats.total_feedback > 0">
        <el-descriptions :column="3" border>
          ...
          <el-descriptions-item label="好评数">
            <el-tag type="success"
              >好评 {{ feedbackStats.positive_count }}</el-tag
            >
          </el-descriptions-item>
          <el-descriptions-item label="差评数">
            <el-tag type="danger"
              >差评 {{ feedbackStats.negative_count }}</el-tag
            >
          </el-descriptions-item>
          <el-descriptions-item label="满意度">
            <el-progress
              ...
              :text-inside="true"
              :color="
                feedbackStats.satisfaction_rate >= 0.8
                  ? '#67c23a'
                  : feedbackStats.satisfaction_rate >= 0.5
                    ? '#e6a23c'
                    : '#f56c6c'
              "
            />
        ...
        <div
          v-if="
            feedbackStats.recent_negative &&
            feedbackStats.recent_negative.length
          "
        >
          <el-divider content-position="left">最近差评</el-divider>
          <div
            v-for="(item, idx) in feedbackStats.recent_negative"
            :key="idx"
            class="negative-item"
          >
    ...
    <el-card class="section-card">
      <template #header>
        <div class="card-header">
          <span
            ><el-icon><DataAnalysis /></el-icon> 官方 RAGAS 评估结果</span
          >
          <el-button
            type="primary"
            size="small"
      ...
      <p class="section-desc">
        展示最近一次【官方 RAGAS】(ragas 0.4.3, LLM-as-Judge) 评测的四项指标（由
        backend/eval/run_comprehensive_ragas.py 生成）
      </p>
      ...
      <div v-if="ragasResult && ragasResult.metrics" class="ragas-result">
        <div class="ragas-metrics">
          <div class="metric-card">
            ...
            <div
              class="metric-value"
              :style="{ color: scoreColor(ragasResult.metrics.faithfulness) }"
            >
          ...
          <div class="metric-card">
            ...
            <div
              class="metric-value"
              :style="{
                color: scoreColor(ragasResult.metrics.answer_relevancy),
              }"
            >
          ...
          <div class="metric-card">
            ...
            <div
              class="metric-value"
              :style="{
                color: scoreColor(ragasResult.metrics.context_precision),
              }"
            >
          ...
          <div class="metric-card">
            ...
            <div
              class="metric-value"
              :style="{ color: scoreColor(ragasResult.metrics.context_recall) }"
            >
          ...
          <div class="metric-card metric-card-overall">
            ...
            <div
              class="metric-value"
              :style="{ color: scoreColor(ragasResult.overall) }"
            >
        ...
        <div class="ragas-info">
          <el-tag size="small" type="success"
            >官方 ragas 0.4.3 (LLM-as-Judge)</el-tag
          >
          <el-tag size="small"
            >judge: {{ ragasResult.judge_model || "N/A" }}</el-tag
          >
          <el-tag size="small" type="info"
            >{{ ragasResult.samples }} 条样本</el-tag
          >
          <el-tag size="small" type="info">{{ ragasResult.date || "" }}</el-tag>
        </div>
      </div>
      <el-empty
        v-else-if="!ragasLoading"
        description="点击「加载官方 RAGAS」显示最近一次官方评测结果（需先运行 backend/eval/run_comprehensive_ragas.py 生成）"
      />
    </el-card>
    ...
    <el-card class="section-card">
      <template #header>
        <div class="card-header">
          <span
            ><el-icon><Medal /></el-icon> LLM-as-Judge 质量评判</span
          >
        </div>
      </template>
      <p class="section-desc">
        使用大模型评判生成答案的质量（答案正确性/相关性/上下文利用/幻觉程度）
      </p>
      ...
      <div v-if="judgeResult" class="eval-result">
        <el-descriptions :column="2" border>
          <el-descriptions-item label="答案正确性">
            <el-rate
              v-model="judgeResult.answer_correctness"
              disabled
              :max="5"
            />
          </el-descriptions-item>
          ...
          <el-descriptions-item label="上下文利用">
            <el-rate
              v-model="judgeResult.context_utilization"
              disabled
              :max="5"
            />
          </el-descriptions-item>
          ...
          <el-descriptions-item label="综合评分" :span="2">
            <el-tag type="success" size="large"
              >{{ judgeResult.overall_score }}/5</el-tag
            >
          </el-descriptions-item>
    ...
    <el-card class="section-card">
      <template #header>
        <div class="card-header">
          ...
          <el-tag size="small" type="info"
            >官方 RAGAS (ragas 0.4.3, DeepSeek judge, 20条)</el-tag
          >
        </div>
      </template>
      ...
      <template #header>
        <div class="card-header">
          ...
          <el-tag size="small" type="warning"
            >示例：向量分 vs BM25分 → 融合分 → 精排分</el-tag
          >
        </div>
      </template>
...
// admin 级别才能访问评估页（与路由 meta.level、后端权限一致）
function canViewEvaluate() {
  return authStore.can("evaluate:view");
}
...
async function getEcharts() {
  if (!echartsModule) {
    try {
      echartsModule = await loadEchartsCDN();
    } catch {
      return null;
    }
  }
...
async function runRagasEval() {
  ...
  try {
    ...
    if (res.success) {
      ...
      const m = res.metrics || {};
      ElMessage.success(
        `官方 RAGAS(judge=${res.judge_model || "N/A"}): F=${(m.faithfulness * 100).toFixed(1)}% / AR=${(m.answer_relevancy * 100).toFixed(1)}%`
      );
    } else {
...
onMounted(() => {
  // admin 级别兜底（与路由守卫 meta.level 双保险）：非 admin 直接回概览页
  if (!canViewEvaluate()) {
    ElMessage.warning("仅管理员可访问 RAG 评估页");
...
async function initDiagnosisChart() {
  ...
  const rerankScores = [0.3, 0.85, 0.48, 0.72, 0.25];
  ...
  const option = {
    ...
    legend: {
      data: ["向量检索分", "BM25分", "RRF融合分", "Reranker精排分"],
      bottom: 0,
    },
...
@media (max-width: 767px) {
  ...
  .evaluate-page .el-card {
    margin-bottom: 10px;
  }
  .evaluate-page .el-row {
    flex-direction: column;
  }
  .evaluate-page .el-col {
    max-width: 100%;
    flex: 0 0 100%;
    margin-bottom: 10px;
  }
  .evaluate-page .el-table {
    font-size: 11px;
  }
}
</style>
