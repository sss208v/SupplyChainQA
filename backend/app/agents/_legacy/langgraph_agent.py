"""
SupplyChainRAG - LangGraph Agent 实现
============================================================
    ...
    for name, tool in available_tools.items():
        ...
        if hasattr(tool, "get_input_schema") and callable(tool.get_input_schema):
            try:
                ...
                logger.debug(f"[LangGraph] 工具 {name} schema 提取失败: {e}")
        lines.append(f"- {name}: {desc}  参数: {params}")
    return "\n".join(lines)


def _parse_json_markdown(raw: str) -> dict:
    """从 LLM 输出中鲁棒解析 JSON — 委托给 app.core.utils.parse_llm_json。
...
async def _call_llm(state: AgentState, available_tools: dict[str, BaseTool]) -> dict:
    ...
    # 尝试解析 LLM 返回的 JSON（带 markdown 容错）
    try:
        parsed = _parse_json_markdown(raw)
    ...
    async def run(
        ...
        if session_id and chat_memory:
            try:
                ...
                logger.warning(f"[LangGraph] 聊天历史加载失败 session={session_id}: {e}")
