...
# ---- 2. 启动 Docker Compose（仅基础设施）----
Write-Host "[2/7] 启动 Docker 服务 (Milvus + Redis + PostgreSQL)..." -ForegroundColor Yellow
...
# ---- 3. 等待服务健康 ----
Write-Host "[3/7] 等待基础设施就绪..." -ForegroundColor Yellow
...
# ---- 4. 启动后端 ----
Write-Host "[4/7] 启动后端 (uvicorn :8001)..." -ForegroundColor Yellow
...
# ---- 5. 启动前端 ----
Write-Host "[5/7] 启动前端 (vite :3000)..." -ForegroundColor Yellow
...
# ---- 6. 检查知识库索引 ----
Write-Host "[6/7] 检查知识库索引..." -ForegroundColor Yellow
if ($backendReady) {
    try {
        $healthResp = Invoke-RestMethod -Uri "http://localhost:8001/health" -TimeoutSec 5
        $docCount = $healthResp.knowledge_docs_count
        if ($docCount -and $docCount -gt 100) {
            Write-Host "  ✅ 知识库已索引: $docCount chunks" -ForegroundColor Green
        } else {
            Write-Host "  ⚠️  知识库 chunks 较少 ($docCount)，需要索引" -ForegroundColor Yellow
            $uploadScript = Join-Path $SCRIPTS_DIR "upload_knowledge_base.py"
            if (Test-Path $uploadScript) {
                Write-Host "  正在运行索引脚本..." -ForegroundColor Yellow
                $uploadResult = & $pythonCmd $uploadScript 2>&1
                Write-Host $uploadResult
            } else {
                Write-Host "  ⚠️  找不到索引脚本: $uploadScript" -ForegroundColor Yellow
                Write-Host "     手动索引: cd backend && $pythonCmd ..\scripts\upload_knowledge_base.py" -ForegroundColor Gray
            }
        }
    } catch {
        Write-Host "  ⚠️  无法检查知识库状态" -ForegroundColor Yellow
    }
} else {
    Write-Host "  ⚠️  后端未就绪，跳过知识库检查" -ForegroundColor Yellow
    Write-Host "     后端启动后手动索引: cd backend && $pythonCmd ..\scripts\upload_knowledge_base.py" -ForegroundColor Gray
}

# ---- 7. 打开浏览器 ----
Write-Host "[7/7] 打开浏览器..." -ForegroundColor Yellow
try {
...
Write-Host "  手册: docs/interview-showcase.html" -ForegroundColor White
Write-Host "  问题: plan/demo-questions.md" -ForegroundColor White
Write-Host "======================================" -ForegroundColor Cyan
Write-Host ""
